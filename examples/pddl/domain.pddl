...
(define (domain BLOCKS)
	...
	; (:derived (p_not_once-tt) (not (top)))
	; (:derived (p_not_onBA) (not (onBA)))
	; (:derived (p_not_ontableC) (not (ontableC)))
	; (:derived (p_not_Y-ontableC) (not (p_Y-ontableC)))
	; (:derived (p_not_onBA-and-Y-ontableC) (not (p_onBA-and-Y-ontableC)))
	...
	(:action prog
		...
		; (when (p_not_ontableC) (not (Y-ontableC)))
		(when (p_Y-ontableC) (Y-ontableC))
		; (when (p_not_Y-ontableC) (not (Y-ontableC)))
		(when (p_onBA-and-Y-ontableC) (onBA-and-Y-ontableC))
		; (when (p_not_onBA-and-Y-ontableC) (not (onBA-and-Y-ontableC)) )
	...
	(:action check
		:parameters ()
		:precondition (and (onBA-and-Y-ontableC))
		:effect (and 
		(goal)
		)
	)
